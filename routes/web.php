...
Route::controller(RepAksatController::class)->group(function (){
  ...
  route::get('/pdfmaincont2/{no}', 'PdfMainCont2')->name('pdfmaincont2') ;
  ...
  route::get('/pdfksm/{bank_no?}/{rep_date1?}/{rep_date2?}/{bank_name?}/{RepRadio?}/{baky?}', 'PdfKsm')->name('pdfksm') ;
  route::get('/pdfbankone/{by?}/{TajNo?}/{bank_no?}/{column?}/{sort?}', 'PdfBankOne')->name('pdfbankone') ;
...
Route::controller(StoresController::class)->group(function (){
  ...
  route::get('/itemdamage','ItemDamage')->name('itemdamage')->middleware('auth');
